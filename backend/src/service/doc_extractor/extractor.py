...
class DocumentExtractor:
    ...
    def draw_bounding_boxes(self, parse_response, document_path, document_type, output_dir=None):
        """Draw bounding boxes around each chunk; if any error occurs, return the original image."""
        def create_annotated_image(image, groundings, page_num=0):
            ...
            for gid, grounding in groundings.items():
                try:
                    ...
                    # Skip invalid boxes
                    if x2 <= x1 or y2 <= y1:
                        self.logger.warning(f"Skipping invalid box for {gid}: {(x1, y1, x2, y2)}")
                        continue

                    # Draw bounding box
                    color = CHUNK_TYPE_COLORS.get(getattr(grounding, "type", "UNKNOWN"), (128, 128, 128))
                    draw.rectangle([x1, y1, x2, y2], outline=color, width=3)

                    # Draw label
                    label = f"{getattr(grounding, 'type', 'UNKNOWN')}:{gid}"
                    label_y = max(0, y1 - 20)
                    draw.rectangle([x1, label_y, x1 + len(label) * 8, y1], fill=color)
                    draw.text((x1 + 2, label_y + 2), label, fill=(255, 255, 255))

                except Exception as e:
                    self.logger.warning(f"Failed to draw box for {gid}: {e}")
                    continue
        ...
        try:
            ...
            if document_path.suffix.lower() == '.pdf':
                ...
                for page_num in range(total_pages):
                    ...
                    try:
                        ...
                        self.logger.warning(f"Failed to draw boxes on page {page_num + 1}: {e}")
                        annotated_img = img  # return original image instead
                ...
                try:
                    ...
                    self.logger.warning(f"Failed to draw boxes: {e}")
                    annotated_img = img  # return original image instead

                annotated_path = Path(output_dir) / f"{document_type}_page_1.png"
                annotated_img.save(annotated_path)
                self.logger.info(f"Annotated image saved to: {annotated_path}")
            ...
            self.logger.warning(f"Failed to draw bounding boxes: {e}")
            # Return normal (unannotated) image in case of any fatal error
            try:
                img = Image.open(document_path)
                if img.mode != "RGB":
                    img = img.convert("RGB")
                ...
                self.logger.error(f"Failed to load fallback image: {inner_e}")
                return None
